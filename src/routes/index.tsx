...
function RotatingInterest({ reduceMotion }: { reduceMotion: boolean }) {
  const interests = ["AI & Data Science", "Machine Learning", "Data Analytics", "Generative AI"];
  ...
  return <div className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-card px-5 py-3 clay-shadow-sm ring-1 ring-border/70"><ArrowRightMark /><AnimatePresence mode="wait"><motion.span key={interests[index]} initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -7 }} className="font-mono text-sm font-bold">{interests[index]}</motion.span></AnimatePresence></div>;
}
...
function Node({ label, position, tone }: { label: string; position: string; tone: string }) { return <div className={cn("float-node absolute grid size-14 place-items-center rounded-2xl font-mono text-xs font-bold clay-shadow-sm", position, `bg-${tone}`)}>{label}</div>; }
