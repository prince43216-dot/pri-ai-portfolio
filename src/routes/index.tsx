...
function CommandPalette({ open, onClose, onGoTo, onToggleTheme, onDownload }: { open: boolean; onClose: () => void; onGoTo: (id: string) => void; onToggleTheme: () => void; onDownload: () => void }) {
  const [query, setQuery] = useState("");
  const [selectedIndex, setSelectedIndex] = useState(0);
  const inputRef = useRef<HTMLInputElement>(null);
  const commands = [...navItems.map(([label, id]) => ({ label: `Go to ${label}`, action: () => onGoTo(id), icon: ArrowDown })), { label: "Download Resume", action: onDownload, icon: FileDown }, { label: "Toggle Theme", action: onToggleTheme, icon: Moon }];
  ...
  const run = (index: number) => { const command = filtered[index]; if (!command) return; command.action(); onClose(); };
  const onKeyDown = (event: React.KeyboardEvent) => {
    if (event.key === "Escape") onClose();
    if (event.key === "ArrowDown") { event.preventDefault(); setSelectedIndex((value) => filtered.length ? (value + 1) % filtered.length : 0); }
    if (event.key === "ArrowUp") { event.preventDefault(); setSelectedIndex((value) => filtered.length ? (value - 1 + filtered.length) % filtered.length : 0); }
    if (event.key === "Enter") run(selectedIndex);
  };

  return <div className="fixed inset-0 z-50 grid place-items-start bg-ink/30 p-4 pt-[15vh] backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Command palette" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className="w-full max-w-xl overflow-hidden rounded-3xl bg-card ring-1 ring-border clay-shadow"><div className="flex items-center gap-3 border-b border-border/70 px-5 py-4"><Command className="size-5 text-indigo" aria-hidden="true" /><input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={onKeyDown} placeholder="Search commands…" className="flex-1 bg-transparent text-sm outline-none placeholder:text-muted-foreground" aria-label="Search commands" aria-activedescendant={filtered[selectedIndex] ? `command-${selectedIndex}` : undefined} role="combobox" aria-expanded="true" /><kbd className="hidden rounded-md bg-surface px-2 py-1 font-mono text-[10px] text-muted-foreground sm:block">ESC</kbd></div><div className="max-h-80 overflow-y-auto p-2" role="listbox">{filtered.map((command, index) => { const Icon = command.icon; const selected = index === selectedIndex; return <button type="button" id={`command-${index}`} role="option" aria-selected={selected} key={command.label} onMouseEnter={() => setSelectedIndex(index)} onClick={() => run(index)} className={cn("flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm transition-colors", selected ? "bg-surface" : "hover:bg-surface")}><Icon className="size-4 text-indigo" aria-hidden="true" /><span>{command.label}</span><ChevronRight className="ml-auto size-4 text-muted-foreground" aria-hidden="true" /></button>; })}{!filtered.length && <p className="px-4 py-8 text-center text-sm text-muted-foreground">No commands found.</p>}</div><div className="flex items-center gap-2 border-t border-border/70 px-5 py-3 text-[11px] text-muted-foreground"><Keyboard className="size-3.5" aria-hidden="true" /> Use ↑ ↓ to navigate and Enter to run</div></div></div>;
}
...
function ProjectDialog({ project, onClose }: { project: Project | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onKey = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(panelRef.current.querySelectorAll<HTMLElement>("button, a[href], input, textarea, select, [tabindex]:not([tabindex='-1'])")).filter((element) => !element.hasAttribute("disabled"));
